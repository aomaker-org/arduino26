#!/usr/bin/env bash
# ==============================================================================
# Path:        tools/upload_sketch.sh
# Purpose:     CLI helper to compile and upload Arduino sketches via arduino-cli or avrdude
# Target OS:   Ubuntu 24.04 / 26.04 LTS (WSL2 / Linux Native)
# Lineage:     Arduino26 Infrastructure
# Updated:     2026-07-31
# ==============================================================================

set -euo pipefail

SKETCH_PATH="${1:-sketches/uno_blink}"
PORT="${2:-/dev/ttyUSB0}"
FQBN="${3:-arduino:avr:uno}"

echo "=========================================================="
echo "    Arduino26 CLI Compile & Upload Helper                 "
echo "=========================================================="
echo "[*] Target Sketch : ${SKETCH_PATH}"
echo "[*] Serial Port   : ${PORT}"
echo "[*] Board FQBN    : ${FQBN}"
echo "----------------------------------------------------------"

# 1. Verify Serial Port Presence
if [ ! -e "${PORT}" ]; then
    echo "[!] WARNING: Serial port '${PORT}' not detected in WSL2." >&2
    echo "    If device is plugged into Windows host, attach it via PowerShell:" >&2
    echo "        pwsh.exe -Command \"usbipd list\"" >&2
    echo "        pwsh.exe -Command \"usbipd attach --wsl --busid <BUSID>\"" >&2
    echo "----------------------------------------------------------"
fi

# 2. Check for arduino-cli
if command -v arduino-cli >/dev/null 2>&1; then
    echo "[+] Utilizing arduino-cli for compilation and upload..."
    arduino-cli compile --fqbn "${FQBN}" "${SKETCH_PATH}"
    echo "[+] Compilation successful. Uploading..."
    arduino-cli upload -p "${PORT}" --fqbn "${FQBN}" "${SKETCH_PATH}"
    echo "[SUCCESS] Sketch uploaded successfully!"
    exit 0
fi

# 3. Fallback to avrdude if pre-compiled .hex exists
HEX_FILE=$(find "${SKETCH_PATH}" -name "*.hex" 2>/dev/null | head -n 1 || true)
if [ -n "${HEX_FILE}" ] && command -v avrdude >/dev/null 2>&1; then
    echo "[*] arduino-cli absent. Falling back to avrdude with pre-compiled HEX..."
    avrdude -C /etc/avrdude.conf -v -p atmega328p -c arduino -P "${PORT}" -b 115200 -D -U flash:w:"${HEX_FILE}":i
    echo "[SUCCESS] HEX flashed via avrdude!"
    exit 0
fi

# 4. Instructions if toolchain is missing
echo "[X] Error: 'arduino-cli' is not installed." >&2
echo "    Install arduino-cli via:" >&2
echo "        curl -fsSL https://raw.githubusercontent.com/arduino/arduino-cli/master/install.sh | sh" >&2
echo "        arduino-cli core update-index && arduino-cli core install arduino:avr" >&2
exit 1

# file tools/upload_sketch.sh ends
