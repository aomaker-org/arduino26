...
def audit_and_connect_arduino():
    print("==========================================================", flush=True)
    print("   Arduino USB Serial Scanner & WSL2 Passthrough Bridge  ", flush=True)
    print("==========================================================", flush=True)

    root = Path(__file__).resolve().parent.parent
    log_dir = root / "agy" / "log"
    ...
    if local_ports:
        for p in local_ports:
            ...
            is_arduino = "arduino" in desc_lower or "ch340" in desc_lower or "usb" in desc_lower or "ttyacm" in p["device"].lower() or "ttyusb" in p["device"].lower()
            tag = " [ARDUINO / CH340 MATCH]" if is_arduino else ""
    ...
    if arduino_port:
        ...
        print("    To bind a physical Arduino / CH340 plugged into Windows host to WSL2:")
        print("    1. Open PowerShell on Windows host:  pwsh.exe")
        print("    2. List connected USB devices:       usbipd list")
        print("    3. Attach device to WSL2:            usbipd attach --wsl --busid <BUSID>")
